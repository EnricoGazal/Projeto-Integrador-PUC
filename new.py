...
class Funcoes():
    ...
    def retorna_produto(self, cod_produto):
        try:
            ...
            messagebox.showerror('Erro',f'\nERRO AO VERIFICAR PRODUTO, VERIFIQUE SE O CAMPO DO CÓDIGO DO PRODUTO ESTEJA PREENCHIDO E SOMENDE COM VALORES NUMÉRICOS: {e}\n')
    ...
    def atualizar(self, cod_produto):
        try:
            ...
            print(f"Novos valores: CP={self.novo_CP}, CF={self.novo_CF}, CV={self.novo_CV}, IV={self.novo_IV}, ML={self.novo_ML}")
        
            self.executor_sql.execute('UPDATE PRODUTOS SET CP = %s, CF = %s, CV = %s, IV = %s, ML = %s WHERE Cod_produto = %s', (self.novo_CP, self.novo_CF, self.novo_CV, self.novo_IV, self.novo_ML, cod_produto))
            self.conexao_bd.commit()
